...
/**
 * Provide functions to check a String for chemically correct syntax
 */
public class FormulaChecker {
    ...
    public boolean checkFormula(String formula) {
        ...
        // Iterates through the string checking its characters
    ...
    private void processUpperCase(char c) {
        assert Character.getType((int)c)==Character.UPPERCASE_LETTER;
        if(upperCaseValid) {
    ...
    /**
     * Sets all the logic to its initial state so before a check is performed with this object
     */
